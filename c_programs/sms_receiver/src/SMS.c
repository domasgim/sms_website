...
struct SMS_Struct PDUDecoding(const char *data) {
    ...
    // Modified part; Adds uint8_t UDH data
    PDU_DESC pduDesc;
	uint8_t errorType;
    memset(&pduDesc, 0x00, sizeof(pduDesc));
    DecodePduData(data, &pduDesc, &errorType);

    sms.udhLen = pduDesc.udhLen;
    sms.udhInfoType = pduDesc.udhLen;
    sms.udhInfoLen = pduDesc.udhInfoLen;
    sms.concateMsgRefNo = pduDesc.concateMsgRefNo;
    sms.concateTotalParts = pduDesc.concateTotalParts;
    sms.concateCurntPart = pduDesc.concateCurntPart;
    sms.isConcatenatedMsg = pduDesc.isConcatenatedMsg;
